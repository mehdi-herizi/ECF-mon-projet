...
<?php require_once ROOT . 'app/views/partials/header.php'; ?>
    <main class="max-w-7xl mx-auto p-6 md:p-12 space-y-8">
        ...
        <div class="bg-gray-900 border border-white/10 rounded-3xl p-8 flex flex-col md:flex-row items-center gap-8 shadow-2xl">
            <img src="<?= htmlspecialchars($avatar) ?>" class="w-32 h-32 rounded-full object-cover border-4 border-purple-600/30 shadow-lg shadow-purple-500/10">
                    ...
                    <?php else: ?>
                        <?php foreach ($orders as $order): ?>
                            <div class="bg-black/40 p-4 rounded-2xl border border-white/5">
                                ...
                                <p class="text-right text-green-400 font-black mt-2"><?= number_format($order['total_calcule'], 2) ?> €</p>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($wishlist as $item): ?>
                            <div class="bg-black/40 p-3 rounded-2xl border border-white/5 flex flex-col items-center text-center">
                                <img src="<?= htmlspecialchars($item['picture']) ?>" class="w-full h-20 object-cover rounded-xl mb-2">
