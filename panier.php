        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                ?>
                    <div class="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[30px] flex items-center justify-between group hover:border-blue-500/50 transition-all duration-300 shadow-xl">
                        <div class="flex items-center gap-6">
                            <img src="<?= $jeu['picture'] ?>" class="w-24 h-24 object-cover rounded-2xl shadow-lg border-2 border-transparent group-hover:border-blue-500 transition-all">
                            <div>
                        ...
                        <a href="panier_action.php?remove=<?= $id ?>" class="bg-red-500/10 hover:bg-red-600 text-red-500 hover:text-white p-4 rounded-2xl transition-all duration-300 shadow-lg shadow-red-900/20">
            ...
            <?php else: ?>
                <div class="bg-white/5 border-2 border-dashed border-white/10 p-20 rounded-[40px] text-center">
                    ...
                    <a href="catalogue.php" class="inline-block mt-8 bg-blue-600 hover:bg-blue-700 px-10 py-4 rounded-full font-black uppercase text-xs tracking-widest text-white shadow-lg shadow-blue-600/30 transition-all transform hover:scale-105">
                        Découvrir les jeux
                    </a>
                </div>
            <?php endif; ?>
            ...
            <div class="bg-blue-600 p-8 rounded-[40px] shadow-2xl shadow-blue-900/50 sticky top-32 transform hover:-translate-y-1 transition-transform border border-white/20">
                ...
                <?php if (!empty($_SESSION['panier'])): ?>
                    <?php if (isset($_SESSION['id_user'])): ?>
                        <a href="valider_commande.php" class="block text-center w-full bg-white text-blue-600 font-black py-5 rounded-2xl uppercase tracking-widest hover:bg-gray-100 transition-all shadow-xl transform active:scale-95">
                            Confirmer l'achat
                        </a>
                    <?php else: ?>
                        <a href="connexion.php" class="block text-center w-full bg-gray-900 text-white font-black py-5 rounded-2xl uppercase tracking-widest hover:bg-black transition-all shadow-xl border border-white/10">
                            Se connecter
                        </a>
                    <?php endif; ?>
                <?php endif; ?>
